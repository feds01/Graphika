...
class GridOptions {
  constructor(options) {
    this.gridded = !utils.isUndefOrNull(options.gridded)
      ? options.gridded
      : true;
    ...
    this.sharedAxisZero = !utils.isUndefOrNull(options.sharedZero)
      ? options.sharedZero
      : true;

    /* This will ensure that the X-Axis grid square length is an integer. */
    this.optimiseSquareSize = !utils.isUndefOrNull(options.optimiseSquareSize)
      ? options.sharedZero
      : true;
  }
}
